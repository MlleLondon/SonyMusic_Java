...
delimiter $
create procedure insertLabel(IN p_nom varchar(100), IN p_email varchar(250), IN p_mdp varchar(250), IN p_telephone varchar(50), IN p_role varchar(50) , IN p_adresse varchar(250), IN p_nbEmployes int(6)) 
Begin 
        Declare p_iduser int(3); 
        
        insert into user values (null, p_nom, p_email, p_mdp, p_telephone, p_role); 
        
        select iduser into p_iduser from user 
...
delimiter $
create procedure insertArtiste (IN p_nom varchar(100), IN p_email varchar(250), IN p_mdp varchar(250), p_telephone varchar(50), IN p_role varchar(50) , IN p_prenom varchar(50), IN p_nomDeScene varchar(50), IN p_typePrincipal varchar(50), IN p_idagent int(3)) 
Begin 
        Declare p_iduser int(3); 
        ...
        from user 
        where nom = p_nom and email=p_email and telephone =p_telephone and role= p_role; 
        insert into artiste values (p_iduser, p_prenom, p_nomDeScene, p_typePrincipal, p_idagent);
End $
delimiter  ;
...
delimiter $
create procedure updateArtiste(IN p_iduser int(3), IN p_nom varchar(100), IN p_email varchar(250), IN p_mdp varchar(50), p_telephone varchar(50), IN p_role varchar(50) , IN p_prenom varchar(50), IN p_nomDeScene varchar(50), IN p_typePrincipal varchar(50), IN p_idagent int(3))
Begin
    update user set nom = p_nom, email = p_email, mdp = p_mdp, telephone = p_telephone, role =p_role 
        where iduser = p_iduser ; 

    update artiste set prenom= p_prenom, nomDeScene= p_nomDeScene, typePrincipal= p_typePrincipal, idagent= p_idagent where iduser = p_iduser ;
End $
delimiter ;
...
create view vueArtistes as (
	select u.*, a.prenom, a.nomDeScene, a.typePrincipal, a.idagent from user u, artiste a where u.iduser=a.iduser
);
...
call insertArtiste("MENDY", "dmendy@gmail.com", "4567", "875745745", "artiste", "David", "DaveLeBg", "Rap FR", 4);
